              ...
              >
                <template v-slot:[`item.quantity`]="{ item }">
                  <v-text-field
                    ...
                    type="number"
                    :value="item.quantity"
                     @keyup="qtyChange($event.target.value,item,sellItems.indexOf(item))"
                  ></v-text-field>
                </template>
              </v-data-table>
            </v-col>
...
export default {
  ...
  computed: {
    ...
    dialog() {
      return this.$store.getters.modaltype == "returnSell"
  ...
  methods: {
    ...
    async submitForm() {
      await this.$axios
        .patch(`/sell-return/${this.item.id}`,{
          sell_return:this.selected
          })
        .then(res => {
          let data = {
            alert: true,
            message: "Sell return successfully",
            type: "success"
          };
          this.selected=[]
